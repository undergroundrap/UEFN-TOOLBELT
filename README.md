# UEFN TOOLBELT
**250 Professional Tools for UEFN Python Integration.**
...
in the UEFN editor bar. **250 registered tools** across 38 categories, complete AI-agent
...
4.  **Export Tool Manifest**: Run `tb.run("plugin_export_manifest")` to write `Saved/UEFN_Toolbelt/tool_manifest.json` — a machine-readable index of all 250 tools with their parameter signatures, types, defaults, and categories. An AI agent can load this file and immediately know what every tool does and how to call it, without reading source code.
...
**163 tests across 250 tools** — covering materials, bulk ops, patterns, scatter, splines, snapshots, asset management, Verse tools, screenshots, LODs, arena, measurement, localization, zones, stamps, actor org, proximity placement, advanced alignment, signs, post-process, audio, level health, config, lighting, and world state.
...
| Tests all 250 tools? | Registry only | Live execution |
...
> **250 tools · 38 categories** — all return `{"status": "ok"/"error", ...}` structured dicts.
...
Start the listener in UEFN, then Claude Code (or any MCP client) can run all 250 tools by name, spawn actors, execute arbitrary Python, and more.
...
A dark-themed floating window opens with a left sidebar nav and 250 tools across 38 categories (including an About page).
...
|---|---|---|
| **Find any tool…** | Sidebar (top) | Global — searches all 250 tools across every category by name, description, or tag. Results show a category badge so you know where each tool lives. |
...
- Run any of the 250 tools by name
...
|:---|:---|:---|
| **Tool Layer** | `Saved/UEFN_Toolbelt/tool_manifest.json` | 250 tools — names, params, types, defaults |
...
> and teaching AI what exists. **UEFN Toolbelt is an Execution Engine** — 250 tools for
...
**What Claude can do:**
- Run any of the 250 toolbelt tools by name (`run_toolbelt_tool`)
...
- Every tool name, category, parameter, and usage example across all 250 tools
...
cd UEFN-TOOLBELT
claude
# → Claude knows all 250 tools, all UEFN quirks, and the full test workflow.
...
- **250 tools** across 38 categories
...
- **Structured Returns Everywhere (Phase 21 complete)**: All 250 tools return `{"status": "ok"/"error", ...}` dicts. Zero `None` returns remain. MCP callers (Claude Code, `client.py`, scripts) can read every result programmatically — no log parsing required.

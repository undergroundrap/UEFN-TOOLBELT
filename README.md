...
[![Version](https://img.shields.io/badge/version-1.9.8-green.svg)](docs/CHANGELOG.md)
...
4.  **Export Tool Manifest**: Run `tb.run("plugin_export_manifest")` to write `Saved/UEFN_Toolbelt/tool_manifest.json` — a machine-readable index of all 296 tools with their parameter signatures, types, defaults, and categories. An AI agent can load this file and immediately know what every tool does and how to call it, without reading source code.
...
**163 tests across 296 tools** — covering materials, bulk ops, patterns, scatter, splines, snapshots, asset management, Verse tools, screenshots, LODs, arena, measurement, localization, zones, stamps, actor org, proximity placement, advanced alignment, signs, post-process, audio, level health, config, lighting, and world state.
...
| Tests all 296 tools? | Registry only | Live execution |
...
> **296 tools · 43 categories** — all return `{"status": "ok"/"error", ...}` structured dicts.
...
Start the listener in UEFN, then Claude Code (or any MCP client) can run all 296 tools by name, spawn actors, execute arbitrary Python, and more.
...
A dark-themed floating window opens with a left sidebar nav and 296 tools across 43 categories (including an About page).
...
|---|---|---|
| **Find any tool…** | Sidebar (top) | Global — searches all 296 tools across every category by name, description, or tag. Results show a category badge so you know where each tool lives. |
...
- Run any of the 296 tools by name
...
|:---|:---|:---|
| **Tool Layer** | `Saved/UEFN_Toolbelt/tool_manifest.json` | 296 tools — names, params, types, defaults |
...
> and teaching AI what exists. **UEFN Toolbelt is an Execution Engine** — 296 tools for
...
**What Claude can do:**
- Run any of the 296 toolbelt tools by name (`run_toolbelt_tool`)
...
- Every tool name, category, parameter, and usage example across all 296 tools
...
cd UEFN-TOOLBELT
claude
# → Claude knows all 296 tools, all UEFN quirks, and the full test workflow.
...
- **296 tools** across 43 categories
...
- **Structured Returns Everywhere (Phase 21 complete)**: All 296 tools return `{"status": "ok"/"error", ...}` dicts. Zero `None` returns remain. MCP callers (Claude Code, `client.py`, scripts) can read every result programmatically — no log parsing required.
